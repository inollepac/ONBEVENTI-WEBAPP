...
import { AppEvent, Attendee, Expense, OnbeDay, PaymentStatus } from '../types';
...
import { 
  ...
  addWaitingListAttendee, deleteWaitingListAttendee, promoteFromWaitingList
...
export const EventDetails: React.FC<EventDetailsProps> = ({ 
  ...
  const handleAddAttendee = async (e: React.FormEvent) => {
    ...
    const isLimitReached = event.maxAttendees && (event.attendees || []).length >= event.maxAttendees;
    ...
    try {
      const updated = isLimitReached 
        ? await addWaitingListAttendee(event.id, attendee)
        : await addAttendee(event.id, attendee);
  ...
  const handlePromoteFromWaiting = async (attendeeId: string) => {
    ...
    setLoading(true);
    try {
      const updated = await promoteFromWaitingList(event.id, attendeeId);
  ...
  const handleRemoveFromWaiting = async (attendeeId: string) => {
    ...
    setLoading(true);
    try {
      const updated = await deleteWaitingListAttendee(event.id, attendeeId);
      if (updated) onUpdate(updated);
    } finally {
      setLoading(false);
    }
  ...
  const priorParticipantKeys = useMemo(() => {
    ...
    const checkPrior = (ev: AppEvent | OnbeDay) => {
      if (ev.id === event.id) return;
      const evDate = new Date(ev.date);
      if (evDate < currentEventDate) {
    ...
    return keys;
  }, [event.id, event.date, allEvents, allOnbeDays]);
  ...
  }, [event.attendees, priorParticipantKeys]);
              ...
              <div className="p-4 md:p-6 bg-gradient-to-r from-indigo-50 to-pink-50 border-b border-pink-100 animate-slide-down">
                ...
                <form onSubmit={handleAddAttendee} className="space-y-4 relative">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 pt-2">
                    <input type="number" step="0.01" className="w-full sm:max-w-[200px] px-4 py-2 border border-gray-200 rounded-lg text-sm font-bold text-pink-600 bg-white h-10" value={newAttendee.paidAmount} onChange={e => setNewAttendee(prev => ({ ...prev, paidAmount: e.target.value }))} placeholder={`Quota (Std: €${event.cost})`} />
                    <div className="flex gap-2">
                ...
                <tbody className="divide-y divide-gray-100">
                  {viewMode === 'attendees' ? (
                   ...
                   waitingList.length === 0 ? (
                    <tr><td colSpan={event.requiresWaiver ? 6 : 5} className="px-6 py-16 text-center text-gray-400">Nessuno in lista d'attesa</td></tr>
                   ) : (
                    waitingList.map(attendee => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-center">
                            <span className="text-gray-400 italic text-sm">€ {attendee.paidAmount?.toFixed(2) || event.cost.toFixed(2)}</span>
                          </td>
                          <td className="px-6 py-4 text-center">
                          ...
                          {event.requiresWaiver && <td className="px-6 py-4 border-l border-gray-50"></td>}
                          <td className="px-6 py-4 text-right">
                          ...
                          <div className="space-y-3" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              <div>
                                ...
                                <input type="number" step="0.01" className="w-full border border-gray-200 rounded-lg text-sm px-3 py-2 mt-1 h-10 font-bold text-pink-600 outline-none focus:ring-2 focus:ring-pink-500" value={editAttendeeData.paidAmount} onChange={e => setEditAttendeeData(p => ({...p, paidAmount: e.target.value}))} placeholder={event.cost.toString()} />
                              </div>
                            </div>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-gray-50" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-400 italic text-xs">Quota std: € {attendee.paidAmount?.toFixed(2) || event.cost.toFixed(2)}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <button 
                              onClick={() => handlePromoteFromWaiting(attendee.id)} 
